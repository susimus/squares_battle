...
class MapEditor(tk_Frame):
    ...
    def init(self):
        ...
        self._current_game_map = GameMap(
            # Debug: Player inside [movable_objects]
            Vector2D(*DEFAULT_RESOLUTION), [], [Player(Vector2D(0, 0))])
    ...
    def _init_slave_widgets(self):
        ...
        self._game_gui.grid(row=0, column=0, rowspan=5)

        self._game_gui.paint_all_game_objects()  # Debug
    ...
    # TODO: Implement [_init_saving_widgets]
    def _init_saving_widgets(self):
        self._filename_entry = tk_Entry(master=self)
        self._filename_entry.grid(row=3, column=1)
    ...
    def _save_button_command(self):
        if self._filename_entry.get() != '':
            with open(self._filename_entry.get(), 'wb') as file_handle:
                pickle_dump(self._current_game_map, file_handle)

                self._current_game_map = GameMap(
                    Vector2D(*DEFAULT_RESOLUTION), [], [])
                self._game_gui.paint_all_game_objects()  # Debug: Not painting?
